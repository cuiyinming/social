...
class DiscoverZanModel extends Model
{
    protected $guarded = [];
    protected $table = 'discover_zan';

    public static function getZanDiscover($user_id, array $idArr)
    {
        $builder = self::where([['user_id', $user_id], ['status', 1]])->whereIn('discover_id', $idArr)->pluck('discover_id');
        if ($builder) {
    ...
    public static function getDataByPage($page, $size, $itemid, $status)
    {
        ...
        if (!is_null($itemid)) {
            $orders->where('discover_id', $itemid)->orWhere('user_id', $itemid);
        }
        if (!is_null($status)) {
